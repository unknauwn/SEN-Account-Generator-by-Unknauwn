...
class iTalk_TextBox_Big : Control
{
    ...
    {
        iTalkTB.AppendText(Text);
    }

    public int Lenght()
    {
        return iTalkTB.TextLength;
    }
...
class iTalk_RichTextBox : Control
{
    ...
    {
        iTalkRTB.SelectionStart = iTalkRTB.TextLength;
        iTalkRTB.SelectionLength = 0;
        iTalkRTB.SelectionColor = color;
        iTalkRTB.AppendText(Environment.NewLine + Text);
        iTalkRTB.SelectionColor = iTalkRTB.ForeColor;
        iTalkRTB.ScrollToCaret();
    }
...
class iTalk_TabControl : TabControl
{
    ...
    private Color _ColorTabSelected;
    public Color ColorTabSelected
    {
        get { return _ColorTabSelected; }
        set
        {
            if (_ColorTabSelected != null)
            {
                _ColorTabSelected = value;
            }
        }
    }
    ...
    public Bitmap ChangeColorImage(Bitmap scrBitmap, Color NewColor)
    {
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        for (int TabIndex = 0; TabIndex <= TabCount - 1; TabIndex++)
        {
            ...
            {
                Rectangle TabRect = new Rectangle(new Point(GetTabRect(TabIndex).Location.X - 2, GetTabRect(TabIndex).Location.Y - 2), new Size(GetTabRect(TabIndex).Width + 3, GetTabRect(TabIndex).Height - 2));
                ...
                Rectangle TabHighlighter = new Rectangle(new Point(GetTabRect(TabIndex).X - 2, GetTabRect(TabIndex).Location.Y - (TabIndex == 0 ? 1 : 1)), new Size(4, GetTabRect(TabIndex).Height - 1));
                _Graphics.FillRectangle(new SolidBrush(Color.FromArgb(89, 169, 222)), TabHighlighter);
                // Draw tab text
                _Graphics.DrawString(TabPages[TabIndex].Text, new Font(Font.FontFamily, Font.Size, FontStyle.Bold), new SolidBrush(Color.FromArgb(254, 255, 255)), new Rectangle(TabRect.Left + 30, TabRect.Top + 12, TabRect.Width - 40, TabRect.Height), new StringFormat { Alignment = StringAlignment.Near });
                ...
                if (this.ImageList != null)
                {
                    ...
                    if (!(Index == -1))
                    {
                        ...
                        _Graphics.SmoothingMode = SmoothingMode.HighQuality;
                        _Graphics.DrawImage(ChangeColorImage((Bitmap)ImageList.Images[TabPages[TabIndex].ImageIndex], SymbolColor), TabRect.X + 9, TabRect.Y + 6, 16, 16);
                    }
                }
            }
            else
            {
                ...
                _Graphics.DrawString(TabPages[TabIndex].Text, new Font(Font.FontFamily, Font.Size, FontStyle.Bold), new SolidBrush(Color.FromArgb(159, 162, 167)), new Rectangle(TabRect.Left + 30, TabRect.Top + 12, TabRect.Width - 40, TabRect.Height), new StringFormat { Alignment = StringAlignment.Near });
